...
# Full2Half.pl
#
# 機能: 全角文字から半角文字への変換テーブルを作成する。
# ・以下の条件をいずれも満たす場合
#   −Halfwidth and Fullwidth Formsブロックに属する（FFE3は除く）
#   −文字名が /^FULLWIDTH/ に合致する
#   −decomposition が１文字
#
# 実行方法:
#   Full2Half.pl -s DIR
...
while (<FIN>) {
    ...
    if ($code =~ /^FF[0-9A-E]/ && # Halfwidth and Fullwidth Forms
