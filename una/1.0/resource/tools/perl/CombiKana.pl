...
# CombiKana.pl
#
# 機能: 以下を対象として仮名の合字データを作成する
# ・UnicodeDataのdecomposition mappingが以下のパターンに合致する
#   −１文字目がひらがなかカタカナで、２文字目が結合文字
# ・上記結合文字を「空白＋結合文字」(準結合文字１)に置き換えたもの
# ・上記準結合文字１を半角変換(準結合文字２)し、更に１文字目のカタカナを
#   半角変換したもの
#
# 実行方法:
#   CombiKana.pl -s DIR -d DIR
...
#   $datadir/TargetKana.dat      - ひらがな、カタカナのコード
...
if ($opt_h || $#ARGV > 0) {
    die <<EOU;
Usage: CombiKana.pl -s DIR -d DIR
...
for ($i = 0; $i < $count; $i ++) {
	...
	if (($combi1 = @combi1[hex($combi)]) !~ /^$/) { # 準結合文字１
	    ...
	    if (($combi2 = @f2h[hex($combi1)]) !~ /^$/ && # 準結合文字２
		($head2 = @f2h[hex($head)]) !~ /^$/) { # ヘッドも半角に変換
		print "$head2 $combi2;$rep\n";
	    }
	}
    } 
    # 仮名の合字では結合文字２文字以上のパターンはない
}
