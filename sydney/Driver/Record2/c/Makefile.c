...
MESSAGE_HDRS = \
	...
	$(HDRDIR)/MessageNumber_VerifyContentStarted.h \
	$(HDRDIR)/MessageNumber_VerifyContentFinished.h \
	...
	$(HDRDIR)/MessageNumber_ExistObjectBeforeTop.h \
	$(HDRDIR)/MessageNumber_ExistObjectAfterLast.h \
	$(HDRDIR)/MessageNumber_BadExpungedList.h \
	$(HDRDIR)/MessageNumber_BadFreeList.h \
	...
	$(HDRDIR)/Message_VerifyContentStarted.h \
	$(HDRDIR)/Message_VerifyContentFinished.h \
	...
	$(HDRDIR)/Message_ExistObjectBeforeTop.h \
	$(HDRDIR)/Message_ExistObjectAfterLast.h \
	$(HDRDIR)/Message_BadExpungedList.h \
	$(HDRDIR)/Message_BadFreeList.h \
...
HDRS = \
	$(TOP_EXPORT_HDRS) \
	$(LOCAL_EXPORT_HDRS) \
	$(HDRDIR)/AreaArchiver.h \
	$(HDRDIR)/AreaManager.h \
	$(HDRDIR)/DataAccess.h \
	$(HDRDIR)/Debug.h \
	$(HDRDIR)/FieldData.h \
	$(HDRDIR)/FieldInfo.h \
	...
	$(HDRDIR)/Manager.h \
	$(HDRDIR)/ObjectID.h \
	$(HDRDIR)/ObjectManager.h \
	$(HDRDIR)/Parameter.h \
	$(HDRDIR)/Utility.h \
	$(HDRDIR)/VariableData.h

MESSAGE_OBJS = \
	Message_VerifyPhysicalFileStarted$O \
	Message_VerifyPhysicalFileFinished$O \
	Message_VerifyContentStarted$O \
	Message_VerifyContentFinished$O \
	Message_VerifyOnGoing$O \
	Message_VerifyFailed$O \
	Message_InconsistentHeader$O \
	Message_DiscordObjectNum$O \
	Message_ExistObjectBeforeTop$O \
	Message_ExistObjectAfterLast$O \
	Message_BadExpungedList$O \
	Message_BadFreeList$O \
...
#ifdef SYD_C_MS7_0
FORCEMULTIPLE = /force:multiple /* ModObject becomes a multiple definition why.*/
#endif
EXTRALOCALDLLFLAGS =  \
	$(FORCEMULTIPLE)
