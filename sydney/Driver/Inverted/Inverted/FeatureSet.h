...
#ifndef __SYDNEY_INVERTED_FEATURESET_H
#define __SYDNEY_INVERTED_FEATURESET_H
...
public:
	//
	//	CLASS
	//	Inverted::FeatureSet::Feature -- 1つの特徴語情報を表すクラス
	//
	struct Feature
	{
		float				m_fWeight;		// 重み
		...
		static ModSize getSize(const ModInvertedFeatureElement& feature_)
		{
			return getSize(feature_.first.getLength());
		}
		...
		static char* dump(char* p,
						  const ModInvertedFeatureElement& feature_)
		{
			Feature* f = syd_reinterpret_cast<Feature*>(p);
			f->m_fWeight = static_cast<float>(feature_.second);
			const ModUnicodeString& s = feature_.first;
...
//
//	CLASS
//	Inverted::FeatureSetPointer -- FeatureSetのポインターを管理するクラス
//
//	NOTES
//
...
public:
	...
	// 代入演算子
	FeatureSetPointer& operator = (FeatureSetPointer& r_)
		{
...
#endif //__SYDNEY_INVERTED_FEATURESET_H
