...
// SearchCapsule.h --
// 
// Copyright (c) 2005, 2006, 2007, 2008, 2009, 2010, 2011, 2023 Ricoh Company, Ltd.
// 
...
#ifndef __SYDNEY_INVERTED_SEARCHCAPSULE_H
#define __SYDNEY_INVERTED_SEARCHCAPSULE_H
...
public:
	...
	// tea構文形式の条件式を取得する
	SYD_INVERTED_FUNCTION
	const ModUnicodeString& getCondition(bool isGetEstimateCount_ = false);
...
private:
	...
	ModTermPool* makeTermPool(bool &isEssential);
	// ModTermPoolを拡張文書から作成
	ModTermPool* makeTermPoolFromExpandDocument(
		int matchMode,
		SearchResultSet &searchResultSet,
		ModTermPool *pool);
	
	// ModTermPoolを自然文から作成 (makeTermPoolの下請け)
	ModTermPool* makePool(const ModUnicodeString& freetext_,
						  const ModLanguageSet& lang_);
	// ModTermPoolを単語リストから作成 (makeTermPoolの下請け)
	ModTermPool* makePool(const WordSetInternal& wordSet_,
						  bool& isEssential_);
	...
	// マッチモードを変換
	static Utility::SearchTermData::MatchMode::Value
	convertTermMatchMode(ModInvertedTermMatchMode eTermMatchMode_);
	static Utility::SearchTermData::MatchMode::Value
	convertTermMatchMode(ModInvertedFileIndexingType eIndexingType_);
	...
	void convertToTeaFormatForGetCount();
	// 自然文をTea構文に変更
	void convertFreeTextToTeaFormat(const ModTermPool* pPool_,
									int iMatchMode_);
	// 必須語をTea構文に変更
	void convertEssentialToTeaFormat(const ModTermPool* pPool_,
									 int iMatchMode_);
	// 必須語以外をTea構文に変更
	void convertNonEssentialToTeaFormat(const ModTermPool* pPool_,
										int iMatchMode_);

	// 検索語リストを取得
	void getSearchTermListForSydneySearch(
		Common::DataArrayData& cSearchTermList_) const;
	static void getSearchTermListForNormalSearch(
	...
	ModTermPool* m_pTermPool;
	ModTermPool* m_pExpandTermPool;
	bool m_bEssentialTerm;
};

_SYDNEY_INVERTED_END
_SYDNEY_END
#endif //__SYDNEY_INVERTED_SEARCHCAPSULE_H
