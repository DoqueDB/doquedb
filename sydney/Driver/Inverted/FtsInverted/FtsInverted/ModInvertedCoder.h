...
// 
// Copyright (c) 1997, 1998, 1999, 2000, 2002, 2004, 2009, 2023 Ricoh Company, Ltd.
// 
...
//
// ※ 符号器に圧縮・伸長の現在位置を保持させるには以下の define を使用する
//
//#define CODER_HAS_CURRENT_LOCATION
...
//
class
ModInvertedCoder : public ModInvertedObject
{
public:
	// パラメータにあった符合器を生成する
	static ModInvertedCoder* create(const ModCharString&);

	virtual void getDescription(ModCharString& description_) const = 0;

#ifdef CODER_HAS_CURRENT_LOCATION
	// 複製を生成する
	virtual ModInvertedCoder* duplicate() const = 0;
#endif

	ModInvertedCoder();
#ifdef CODER_HAS_CURRENT_LOCATION
	ModInvertedCoder(const ModInvertedCoder&);
#endif
...
#ifdef CODER_HAS_CURRENT_LOCATION
	virtual ModBoolean get(ModSize&) = 0;
	virtual ModBoolean getBack(ModSize&) = 0;

	void set(Unit*, const ModSize, const ModSize);
	ModSize getBitOffset() const;
	void setBitOffset(Unit*, const ModSize);
#endif
...
ModInvertedCoder::ModInvertedCoder()
	...
	count(0),
#endif
	Unit(0), dataValue(0), currentLength(0), currentOffset(0)
#else
#ifdef DEBUG
	: count(0)
#endif
...
#ifdef CODER_HAS_CURRENT_LOCATION
inline
ModInvertedCoder::ModInvertedCoder(const ModInvertedCoder& orig_)
	:
#ifdef DEBUG
	count(orig_.count),
#endif
	Unit(orig_.Unit), dataValue(orig_.dataValue),
	currentLength(orig_.currentLength), currentOffset(orig_.currentOffset)
{}
#endif
...
#ifdef CODER_HAS_CURRENT_LOCATION
inline void
ModInvertedCoder::set(Unit* Unit_,
					  const ModSize currentLength_,
					  const ModSize currentOffset_)
{
	currentLength = currentLength_;
	currentOffset = currentOffset_;
	Unit = Unit_ + (currentOffset>>5);
	dataValue = *Unit;
}

inline ModSize 
ModInvertedCoder::getBitOffset() const
{
	return currentOffset;
}

inline void
ModInvertedCoder::setBitOffset(Unit* Unit_,
							   const ModSize currentOffset_)
{
	; ModAssert(currentOffset_ < currentLength);
	currentOffset = currentOffset_;
	Unit = Unit_ + (currentOffset>>5);
	dataValue = *Unit;
}
