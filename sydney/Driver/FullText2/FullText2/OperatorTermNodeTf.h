...
#ifndef __SYDNEY_FULLTEXT2_OPERATORTERMNODETF_H
#define __SYDNEY_FULLTEXT2_OPERATORTERMNODETF_H
...
//
//	CLASS
//	FullText2::OperatorTermNodeTf -- TF値を合成する末端ノード
//
//	NOTES
//
class OperatorTermNodeTf : public OperatorTermNodeScore,
						   public OperatorTermNodeArray
{
public:
	// コンストラクタ
	OperatorTermNodeTf(const ModUnicodeString& cString_);
	// デストラクタ
	virtual ~OperatorTermNodeTf();
	// コピーコンストラクタ
	OperatorTermNodeTf(const OperatorTermNodeTf& src_);
...
private:
	...
	{
		NodeData()
			: m_iField(-1), m_pTermNode(0), m_uiID(0), m_dblScale(0) {}
		NodeData(int iField_, OperatorTermNodeSingle* pTermNode_,
				 double tfScale_)
			: m_iField(iField_),
			  m_pTermNode(pTermNode_), m_uiID(0),
			  m_dblScale(tfScale_) {}
...
#endif //__SYDNEY_FULLTEXT2_OPERATORTERMNODETF_H
