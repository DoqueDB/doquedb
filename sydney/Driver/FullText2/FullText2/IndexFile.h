...
#ifndef __SYDNEY_FULLTEXT2_INDEXFILE_H
#define __SYDNEY_FULLTEXT2_INDEXFILE_H
...
public:
	...
	// コンストラクタ
	IndexFile(Type::Value eType_, int iCacheCount_);
	// デストラクタ
	virtual ~IndexFile();
	...
	// ファイルをクリアする
	virtual void clear(bool bForce_);
...
protected:
	//
	//【注意】
	// protectedなメソッドの多くは、呼出し側でラッチが必要
	//
	
	// ロック名を得る
	const Lock::FileName& getLockName() const { return m_cLockName; }
...
#endif //__SYDNEY_FULLTEXT2_INDEXFILE_H
