...
#ifndef __SYDNEY_BTREE2_PAGEPOINTER_H
#define __SYDNEY_BTREE2_PAGEPOINTER_H
...
public:
	...
	// 代入演算子
	PageObjectPointer<PAGE>& operator = (const PageObjectPointer<PAGE>& cPointer_)
	{
...
private:
	...
	void increment()
	{
		if (m_pPage)
		{
			Os::AutoCriticalSection cAuto(m_pPage->getLatch());
			m_pPage->incrementReference();
		}
	}
	...
	void decrement()
	{
		...
		{
			Os::AutoCriticalSection cAuto(m_pPage->getLatch());
			if (m_pPage->getReferece() == 1)
				m_pPage->detach();
			m_pPage->decrementReference();
		}
	}
...
#endif //__SYDNEY_BTREE2_PAGEPOINTER_H
