...
// vi:set ts=4 sw=4:
//
// KdTreeFile.h -- KdTreeファイルオブジェクト関連のクラス定義、関数宣言
// 
...
#ifndef	__SYDNEY_SCHEMA_KDTREEFILE_H
#define	__SYDNEY_SCHEMA_KDTREEFILE_H
...
//	CLASS
//	Schema::KdTreeFile -- KdTreeファイルオブジェクトを表すクラス
//
//	NOTES
//		KdTreeファイルの親オブジェクトは、データベースである
...
public:
	KdTreeFile();
	KdTreeFile(const Database& database, Table& table, Index& index, const Hint* pHint_ = 0, const Hint* pAreaHint_ = 0);
												// コンストラクター
...
//		KdTreeファイルを表すクラスのデフォルトコンストラクター
//
//	NOTES
//		この KdTreeファイルの名前は新たに生成されずに、
//		空文字列のままである
//
//	ARGUMENTS
//		なし
...
//	FUNCTION public
//	Schema::KdTreeFile::getClassID -- このクラスのクラス ID を得る
//
//	NOTES
//
//	ARGUMENTS
...
getClassID() const
{
	return Externalizable::Category::KdTreeFile +
		Common::Externalizable::SchemaClasses;
}

//	FUNCTION public
//	Schema::KdTreeFile::clear --
//		KdTreeファイルを表すクラスのメンバーをすべて初期化する
//
//	NOTES
//		親クラスのメンバーは初期化しない
//
...
//
//	FUNCTION public
//	Schema::KdTreeFile::getAreaCategory --
//		KdTreeファイルを格納するエリアの種別を得る
//
//	NOTES
//
//	ARGUMENTS
...
#endif	// __SYDNEY_SCHEMA_KDTREEFILE_H
