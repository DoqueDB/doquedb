...
// vi:set ts=4 sw=4:
//
// FullTextFile.h -- 全文ファイルオブジェクト関連のクラス定義、関数宣言
// 
// Copyright (c) 2000, 2001, 2004, 2005, 2007, 2011, 2013, 2023 Ricoh Company, Ltd.
// 
...
#ifndef	__SYDNEY_SCHEMA_FULLTEXTFILE_H
#define	__SYDNEY_SCHEMA_FULLTEXTFILE_H
...
//	CLASS
//	Schema::FullTextFile -- 全文ファイルオブジェクトを表すクラス
//
//	NOTES
//		全文ファイルの親オブジェクトは、データベースである
...
public:
	...
	FullTextFile(const Database& database, Table& table, const Hint* pHint_ = 0, const Hint* pAreaHint_ = 0);
#endif
	FullTextFile(const Database& database, Table& table, Index& index, const Hint* pHint_ = 0, const Hint* pAreaHint_ = 0);
												// コンストラクター
...
//		全文ファイルを表すクラスのデフォルトコンストラクター
//
//	NOTES
//		この 全文ファイルの名前は新たに生成されずに、
//		空文字列のままである
//
//	ARGUMENTS
//		なし
...
//	FUNCTION public
//	Schema::FullTextFile::getClassID -- このクラスのクラス ID を得る
//
//	NOTES
//
//	ARGUMENTS
...
getClassID() const
{
	return Externalizable::Category::FullTextFile +
		Common::Externalizable::SchemaClasses;
}

//	FUNCTION public
//	Schema::FullTextFile::clear --
//		全文ファイルを表すクラスのメンバーをすべて初期化する
//
//	NOTES
//		親クラスのメンバーは初期化しない
//
...
//
//	FUNCTION public
//	Schema::FullTextFile::getAreaCategory --
//		全文ファイルを格納するエリアの種別を得る
//
//	NOTES
//
//	ARGUMENTS
...
#endif	// __SYDNEY_SCHEMA_FULLTEXTFILE_H
