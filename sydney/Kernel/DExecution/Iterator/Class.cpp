...
_SYDNEY_DEXECUTION_ITERATOR_USING
...
getClassInstance(int iClassID_)
{
	...
	switch ( eCategory ) {
	case Category::ServerQuery:
	case Category::ServerExecute:
	case Category::PreparedQuery:
	{
		pObject = Server::getInstance(eCategory);
		break;
	}

	case Category::Expand:
	{
		pObject = Expand::getInstance(eCategory);
		break;
	}
