// -*-Mode: C++; tab-width: 4; c-basic-offset: 4;-*-
// vi:set ts=4 sw=4:
//
// Candidate/Table.h --
// 
// Copyright (c) 2012, 2013, 2023 Ricoh Company, Ltd.
// 
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
// 
//     http://www.apache.org/licenses/LICENSE-2.0
// 
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.
// 

#ifndef __SYDNEY_DPLAN_CANDIDATE_TABLE_H
#define __SYDNEY_DPLAN_CANDIDATE_TABLE_H

#include "DPlan/Module.h"
#include "DPlan/Declaration.h"

#include "Plan/Candidate/Base.h"

_SYDNEY_BEGIN
_SYDNEY_DPLAN_BEGIN
_SYDNEY_DPLAN_CANDIDATE_BEGIN

////////////////////////////////////////////////////
// CLASS
//	DPlan::Candidate::Table -- implementation class of Interface::ICandidate for table
//
// NOTES
class Table
	: public Plan::Candidate::Base
{
public:
	typedef Table This;
	typedef Plan::Candidate::Base Super;

	// for distributed table
	struct Distribute
	{
		struct Retrieve
		{
			// constructor
			static This* create(Opt::Environment& cEnvironment_,
								Plan::Interface::IRelation* pRelation_);
		};
		struct Insert
		{
			// constructor
			static This* create(Opt::Environment& cEnvironment_,
								Plan::Relation::Table* pRelation_,
								Plan::Interface::ICandidate* pOperand_,
								Plan::Interface::IScalar* pCheck_,
								bool bRelocateUpdate_);
		};

		struct Update
		{
			// constructor
			static This* create(Opt::Environment& cEnvironment_,
								Plan::Relation::Table* pRelation_,
								Plan::Interface::ICandidate* pOperand_);
		};		

		struct Delete
		{
			// constructor
			static This* create(Opt::Environment& cEnvironment_,
								Plan::Relation::Table* pRelation_);
		};
	};

	// for replicated table
	struct Replicate
	{
		struct Retrieve
		{
			// constructor
			static This* create(Opt::Environment& cEnvironment_,
								Plan::Interface::IRelation* pRelation_);
		};
		struct Insert
		{
			// constructor
			static This* create(Opt::Environment& cEnvironment_,
								Plan::Relation::Table* pRelation_,
								Plan::Interface::ICandidate* pOperand_);
		};

	};

	// destructor
	virtual ~Table() {}

	// create plan
	virtual Plan::Interface::ICandidate* createPlan(Opt::Environment& cEnvironment_,
													Plan::AccessPlan::Source& cPlanSource_,
													const Plan::Utility::FieldSet& cFieldSet_) = 0;


protected:
	// constructor
	Table()
		: Super()
	{}

private:
};

_SYDNEY_DPLAN_CANDIDATE_END
_SYDNEY_DPLAN_END
_SYDNEY_END

#endif // __SYDNEY_DPLAN_CANDIDATE_TABLE_H

//
//	Copyright (c) 2012, 2013, 2023 Ricoh Company, Ltd.
//	All rights reserved.
//
