...
// vi:set ts=4 sw=4:
//
// UnsignedIntegerData.h --	32 ビット長非負整数型データ関連の
//							クラス定義、関数宣言
// 
...
#ifndef __TRMEISTER_COMMON_UNSIGNEDINTEGERDATA_H
#define __TRMEISTER_COMMON_UNSIGNEDINTEGERDATA_H
...
//	CLASS
//	Common::UnsignedIntegerData -- 32 ビット長の非負整数型のデータを表すクラス
//
//	NOTES

class SYD_COMMON_FUNCTION UnsignedIntegerData
	: public	ScalarData
{
public:
	//コンストラクタ(1)
	UnsignedIntegerData();
	//コンストラクタ(2)
	explicit UnsignedIntegerData(unsigned int uiValue_);
	//デストラクタ
	virtual ~UnsignedIntegerData();
...
//	cast(DataType::Type type) const;
	...
	//値を設定する
	void setValue(unsigned int uiValue_);
...
protected:
	// Nullでないことが保証されるときのgetValue
	unsigned int getValue_NotNull() const;
...
#endif // __TRMEISTER_COMMON_UNSIGNEDINTEGERDATA_H
